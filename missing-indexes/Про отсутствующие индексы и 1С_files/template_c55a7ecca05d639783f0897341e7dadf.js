...
; /* Start:/bitrix/templates/.default/components/bitrix/system.auth.form/head-login-form/script.js*/
$(document).ready(function () {
    $(".open-inf-right-menu").click(function () {
        $("html").css({"overflow":"hidden"});
        $("#inf-right-menu").show().children(".inf-right-menu-wrap").css({"right":"-300px"});
        $("#inf-right-menu").children(".inf-right-menu-wrap").animate({"right":"0px"},400);
    });
    $(".inf-right-menu-close, .inf-right-menu-overlay").click(function () {
        $("html").css({"overflow":""});
        $("#inf-right-menu").children(".inf-right-menu-wrap").animate({"right":"-300px"},400,function () {
            $("#inf-right-menu").hide();
        });
    });
...
/* End */
;
; /* Start:/bitrix/components/infostart/callback/templates/.default/script.js*/
$(document).ready(function () {
    $(".m-callback-close").click(function () {
        $(".m-callback").fadeOut();
        return false;
    });
    $(".m-callback-open").click(function () {
        if($(".m-callback:visible").length > 0){
            $(".m-callback").fadeOut();
        }else {
            $(".m-callback").fadeIn();
        }
    ...
    $("#m-callback").submit(function () {
        $("#m-callback input[name=NUMBER]").css('border-color','');
        if($("#m-callback input[name=NUMBER]").val().length < 10){
            $("#m-callback input[name=NUMBER]").css('border-color','red');
            return false;
        }
        $.get('/bitrix/ajax/callback.php?SESSID='+BX.bitrix_sessid()+'&ACTION=call&NUMBER='+$("#m-callback input[name=NUMBER]").val()+'&NAME='+$("#m-callback input[name=NAME]").val());
    ...
    $("#m-form-request-wrap").submit(function(){
        $("#m-form-request-wrap input, #m-form-request-wrap textarea").css('border-color','');
        formData = new FormData($("#m-form-request-wrap").get(0));
        $.ajax({
            url: '/ajax/send_landing_form.php?ACTION=SEND_FORM&RESPONS=feedback&SESSID='+BX.bitrix_sessid(),
            type: 'post',
            contentType: false,
            processData: false,
            data: formData,
            ...
            success: function(arResult){
                if(arResult.ERROR){
                    $("#m-form-request-wrap input[name="+arResult.FIELD+"], #m-form-request-wrap textarea[name="+arResult.FIELD+"]").css('border-color','#e4aeae');
                }else{
                    $("#m-form-request-wrap").next().show();
                    $("#m-form-request-wrap").remove();
                    setTimeout(function () {
                        $(".m-callback").fadeOut();
                    },3000);
                }
...
/* End */
;
; /* Start:/bitrix/components/infostart/support.ticket.edit/templates/chat/script.js*/
$(document).ready(function () {

	if ($("#support-ticket-create").length > 0){
		// $("#support-ticket-create").addClass('animated');
		...
		function ChechkIsFooterVisible() {
			if (isScrolledIntoView('#support-ticket-wrap')){
				$("#support-ticket-create").addClass('animated')
			}else{
				$("#support-ticket-create").removeClass('animated')
			}
		}

		ChechkIsFooterVisible()
		$(window).on('scroll', ChechkIsFooterVisible)

		// setTimeout(function () {
		//     $("#support-ticket-create .animated-circles").addClass('animated');
		// },30000)

		/*window.AnimateSupportCreate = setInterval(function () {
			$("#support-ticket-create .animated-circles").removeClass('animated');
			setTimeout(function () {
				$("#support-ticket-create .animated-circles").addClass('animated');
			},100);
			if($("#support-ticket-create").length < 1)
				clearTimeout(window.AnimateSupportCreate);
		},5000);*/
	}
});
$(document).on("click",'#support-ticket-create',function() {
	$("#support-ticket-create").hide();
	$("#support-ticket-created").fadeIn();
    $("#support-ticket-message").focus();
});
$(document).on("click",'#support-ticket-wrap .support-ticket-close',function() {
    $.cookie('support-ticket-min', 'N', { expires: 666, path: '/' });
    $.cookie('support-hash', '', { expires: 666, path: '/' });
    $.cookie('support-id', '', { expires: 666, path: '/' });
    var ID = $("#support-ticket-message").attr("data-id");

    if (ID > 0) {
        $("#support-ticket-wrap").html('');
        $.post('', {
            ...
            $("#support-ticket-wrap").html(data);
        });
    }else{
        $("#support-ticket-created").hide();
        $("#support-ticket-create").fadeIn();
...
$(document).on("click",'#support-ticket-wrap .support-ticket-min',function() {
    $("#support-ticket-wrap .ui-chatbox-content").slideToggle(400,function () {
        if ($("#support-ticket-wrap .ui-chatbox-content:visible").length > 0){
            $.cookie('support-ticket-min', 'N', { expires: 666, path: '/' });
        }else{
            $.cookie('support-ticket-min', 'Y', { expires: 666, path: '/' });
        }
    });
...
$(document).on("mouseover",'#support-ticket-create',function() {
	$("#support-ticket-create .js-livechat-hint").removeClass("hide_hint").addClass("show_hint");
});
$(document).on("mouseout",'#support-ticket-create',function() {
    $("#support-ticket-create .js-livechat-hint").removeClass("show_hint").addClass("hide_hint");
});

function SupportSendMessage() {
    if ($("#support-ticket-message").attr("data-id") == 0){
        $.cookie('support-ticket-min', 'N', { expires: 666, path: '/' });
        $.post("/bitrix/ajax/livechat.php?_t="+new Date().getTime(),{
            "ACTION":"LIVE_SUPPORT_TICKET",
            "MESSAGE":$("#support-ticket-message").val()
        },function (data) {
            $("#support-ticket-wrap").html(data);

            if(window.SUPPORT_HASH) {
                $.cookie('support-hash', window.SUPPORT_HASH, {expires: 666, path: '/'});
                $.cookie('support-id', window.SID, {expires: 666, path: '/'});
            }
        ...
        $.post('',{
            ...
            "ACTION":"AJAX_FORM",
            "set_default":"Y",
            "ID":$("#support-ticket-message").attr("data-id"),
            "MESSAGE":$("#support-ticket-message").val(),
            "apply":"Y",
        },function () {
            //console.info(data);
    ...
    $("#support-ticket-message").val('');
}

$(document).on("click",'#support-ticket-send',function() {
    SupportSendMessage();
});

$(document).on("keydown",'#support-ticket-message',function() {
    if (event.keyCode == 13) {
        SupportSendMessage();
        return false;
    }
...
BX.ready(function(){
    BX.addCustomEvent("onPullEvent", function(module_id,command,params) {
        if(module_id == 'support'){
            if(command == 'disable-chat' && $("#support-ticket-message").length > 0 && parseInt($("#support-ticket-message").attr("data-id")) < 1){
                $("#support-ticket-wrap").remove();
            }

            if(command == 'new-message' && window.SID == params.TID && $("#support-ticket-wrap").length > 0) {
                $.get("?ACTION=RENEW_TICKET",function (data) {
                    $("#support-ticket-wrap .ui-widget-header").html($(data).find(".ui-widget-header").html());
                    $("#support-ticket-chat").html($(data).find("#support-ticket-chat").html());

                    if($(data).find(".alert-msg").length < 1){
                        $("#support-ticket-wrap .alert-msg").remove();
                    }
                    /*$("#support-ticket-wrap").html(data);*/
                    $("#support-ticket-chat").scrollTop($("#support-ticket-chat .ui-chatbox-msg:last").offset().top);
                });
            }
        }
    });
...
/* End */
;; /* /bitrix/templates/.default/components/bitrix/system.auth.form/head-login-form/script.js*/
; /* /bitrix/components/infostart/callback/templates/.default/script.js*/
; /* /bitrix/components/infostart/support.ticket.edit/templates/chat/script.js*/
...
/*
     FILE ARCHIVED ON 21:00:55 Jul 08, 2017 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 08:03:29 Sep 10, 2019.
...
playback timings (ms):
  ...
  LoadShardBlock: 48.369 (3)
  CDXLines.iter: 15.019 (3)
  PetaboxLoader3.datanode: 87.693 (5)
  PetaboxLoader3.resolve: 85.603 (2)
  RedisCDXSource: 0.957
  exclusion.robots.policy: 0.265
  load_resource: 150.846
  captures_list: 68.655
*/